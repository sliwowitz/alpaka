...
namespace alpaka
{
    namespace rand
    {
        ...
        ALPAKA_FN_HOST_ACC constexpr static std::uint32_t hi32(std::uint64_t x)
        {
            return static_cast<std::uint32_t>(x >> 32);
        }

        /// Get low 32 bits of a 64-bit number
        ALPAKA_FN_HOST_ACC constexpr static std::uint32_t lo32(std::uint64_t x)
        {
        ...
        ALPAKA_FN_HOST_ACC constexpr static void mulhilo32(
            std::uint64_t a,
            std::uint64_t b,
            ...
            std::uint32_t& resultLow)
        {
            ...
            resultHigh = hi32(res64);
            resultLow = lo32(res64);
        }
    } // namespace rand
} // namespace alpaka
