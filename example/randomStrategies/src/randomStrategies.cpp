...
void runStrategy(Box& box)
{
    ...
    // offset strategy needs an additional parameter for
    // initialisation: the offset cannot be deduced form the size of
    // the PRNG buffer and has to be passed in explicitly
    // other strategies ignore the last parameter, and deduce the initial parameters solely from the thread index
